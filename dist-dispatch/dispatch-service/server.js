...
const getRoutes = async (dep, minLeg, maxLeg, aircraft, company, arr, returnOneLeg) => {
...
apiRouter.get('/getcareeroptionsdata', validateAccessToken, async (req, res) => {
    try {
...
apiRouter.get('/generatedispatch', validateAccessToken, async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < legNumber; i++) {
            if (i === 0) {
                let randomLeg = await getRoutes(base, minLeg, maxLeg, aircraft, company, null, true);
                if (!randomLeg) {
                    res.status(200).send({
                    ...
                    randomLeg.flightNo = randomLeg.flightNo.substring(3, randomLeg.flightNo.length);
                    flights.push(randomLeg);
                }
            }
            else {
                let randomLeg = await getRoutes(flights[i - 1].icaoArr, minLeg, maxLeg, aircraft, company, null, true);
                if (!randomLeg) {
                    res.status(200).send({
                        message: 'Please check your settings and try again',
                    });
        ...
        if (flights[0].icaoDep !== flights[flights.length - 1].icaoArr) {
            ...
            let returningLeg = await getRoutes(flights[flights.length - 1].icaoArr, minLeg, maxLeg, aircraft, company, flights[0].icaoDep, true);
            if (returningLeg) {
                // remove airline code from flight number
                returningLeg.flightNo = returningLeg.flightNo.substring(3, returningLeg.flightNo.length);
                flights.push(returningLeg);
            }
            else {
                let outboundFromLast = await getRoutes(flights[flights.length - 1].icaoArr, minLeg, maxLeg, aircraft, company, null, false);
                if (outboundFromLast.length > 0) {
                    outboundFromLast = randomizeFlights(outboundFromLast);
                    let flight;
                    for (flight of outboundFromLast) {
                        let returnToStart = await getRoutes(flight.icaoArr, minLeg, maxLeg, aircraft, company, flights[0].icaoDep, true);
                        if (returnToStart) {
                            // remove airline code from flight number
                            flight.flightNo = flight.flightNo.substring(3, flight.flightNo.length);
                            returnToStart.flightNo = returnToStart.flightNo.substring(3, returnToStart.flightNo.length);
